...
- 対象範囲は要件定義書 4 章の Phase 1（記録 CRUD・自己ベスト表示・月間カレンダー・週間一覧・種目管理・Google 認証）のみとし、Phase 2 機能（成長グラフ・目標設定・メニュー・allowlist 管理画面）は対象外とする（8 章参照）。
...
未認証状態（ログイン画面）ではナビゲーションを表示しない。認証後の画面（`/`・`/weekly`・`/records/:date`・`/exercises`）は、画面幅を問わず共通の「トグルボタン＋ドロワー（オフキャンバスパネル）」方式でナビゲーションを提供する。

| 要素 | 内容 |
|---|---|
| トグルボタン | 全画面共通のヘッダーに配置するハンバーガーメニュー等のボタン。押下でドロワーの開閉を切り替える |
| ドロワー | 画面端からモーダル的にオーバーレイ表示されるパネル。カレンダー／週間一覧／種目管理 の 3 項目への導線と、ユーザーメニュー（ログアウトボタンを含む）を含む |
| 閉じる操作 | ドロワー外側のオーバーレイ領域のタップ、項目選択、または明示的な閉じるボタンのいずれかで閉じる |

- 画面幅（モバイル／タブレット／PC）によってナビゲーション形式そのものを分岐させない。従来検討した「モバイル：フッタータブ／PC：常時表示サイドバー」の分岐方式は採用しない。理由：単一の実装・単一の見た目で全画面幅に対応でき、設計・実装の分岐を減らせるため。
- ドロワーの具体的な幅・アニメーション・開閉時のフォーカス制御等の詳細は実装時に定める（8 章参照）。
- ログアウト処理は `POST /api/auth/logout` を呼び出し、成功後はログイン画面へ遷移する（auth.md 7 章）。
...
flowchart TB
    ...
    Calendar -- "ドロワーメニュー" --> Weekly["週間一覧画面\n/weekly"]
    Calendar -- "ドロワーメニュー" --> Exercises["種目管理画面\n/exercises"]

    Weekly -- "日付タップ" --> Record
    Weekly -- "ドロワーメニュー" --> Calendar
    Weekly -- "ドロワーメニュー" --> Exercises

    Exercises -- "ドロワーメニュー" --> Calendar
    Exercises -- "ドロワーメニュー" --> Weekly

    Record -- "戻る" --> Calendar
    Record -- "戻る（週間一覧経由の場合）" --> Weekly
    Record -- "ドロワーメニュー" --> Exercises
...
- **ブラウザの戻る/進むボタンへの対応**：SPA内の全ての画面遷移（4章の遷移図に示す全経路）は、ブラウザの戻る/進むボタンで正しく遷移できることを要件とする。実装時はブラウザの History API（`pushState`/`popstate`）に基づくクライアントサイドルーター（例：React Router）を採用し、画面遷移のたびにURLが変化する構成とすることでこれを満たす。ルーティングライブラリの具体的な選定は実装時に定める。
...
| 記録有無の識別 | 該当日に記録が存在する場合、日付セルにマーカー（ドット等）を表示する。データ源は `GET /api/records?from=&to=`（openapi.yaml）のレスポンスとし、各日について `RecordSummary.exerciseCount > 0` で記録有無を判定する（フロントエンドが DB テーブルを直接参照することはしない） |
...
| 当日のハイライト | 5.3 節の月間カレンダー画面と同様に、当日の行を視覚的に強調する |
...
| 画面への導線 | 本画面への遷移は、月間カレンダー画面（5.3 節）・週間一覧画面（5.4 節）の日付タップのみとする。記録の新規作成専用の入口（例：「＋記録を追加」ボタン等）は設けず、日付タップを起点とする単一の導線に統一する |
| 自己ベスト（最高記録）の自動表示 | 画面上部に、その種目の自己ベスト（最大重量）を記録した日に、その種目で行った全セットを、重量／回数／セット番号の3列を持つ表形式で一覧表示する（requirement.md 5.2）。取得は `GET /api/exercises/{exerciseId}/best-set` を用い、各種目ブロックはルートの `:date` を `excludeDate` クエリとして送り、編集中の記録自身を自己ベスト算出から除外する（openapi.yaml）。該当する記録が存在しない場合は「この種目の記録はまだありません」等、記録が一つもないことが伝わるプレースホルダーを表示する |
| 種目・セットの構成 | 1 日の記録は複数の種目を含み、各種目は複数のセットを含む。種目ごとにセット一覧（重量・回数）をブロック表示する |
| 種目の追加（既存種目の選択） | 記録済み・事前定義を問わず、既存の種目一覧（種目管理画面で追加した種目を含む）から選択する |
| 種目の追加（新規種目のその場作成） | 種目選択 UI 内に「新規追加」の導線を設け、モーダルで種目名・カテゴリを入力してその場で登録できるようにする。登録後は画面遷移せずに選択肢へ即座に反映し、そのまま選択状態にできる。入力項目・制約は 5.6 節の種目管理画面と同一とする |
...
| 追加 | 種目名・カテゴリ（8 値から選択）を入力するフォーム（モーダル等）を提供する。5.5 節の記録詳細・編集画面内で提供する「新規追加」モーダルと同一の入力項目・制約を用いる（UI コンポーネントを共用する想定） |
...
5.5 節参照。該当する記録が存在しない場合、空欄ではなく「この種目の記録はまだありません」等、記録が一つも存在しないことが明示的に分かる文言を表示し、データ取得の失敗と区別できるようにする。

### ローディング・エラー表示

API 通信中のローディング表示、通信エラー時の表示方針（トースト等の具体的な UI コンポーネント）は、共通エラーレスポンス形式と合わせて `common-spec.md` で定める。本書では画面固有の表示内容（本章・5 章に記載のメッセージ文言）のみを対象とする。
...
- ドロワーナビゲーションの具体的な幅・アニメーション・開閉時のフォーカス制御等の実装詳細（実装時に確定する）
- クライアントサイドルーティングライブラリの具体的な選定（実装時に定める。History API ベースであることのみを要件とする。4 章参照）
- 共通エラーレスポンス形式・ローディング表示の具体的な UI コンポーネント仕様（`common-spec.md` で定める）
- OpenAPI（リクエスト・レスポンスの具体的なスキーマ）は `openapi.yaml` で定める
