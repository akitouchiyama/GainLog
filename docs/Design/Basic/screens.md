...
| 自己ベスト（最高記録）の自動表示 | 画面上部に、その種目の自己ベスト（最大重量）を記録した日に、その種目で行った全セットを、重量／回数／セット番号の3列を持つ表形式で一覧表示する（requirement.md 5.2）。該当する記録が存在しない場合は「この種目の記録はまだありません」等、記録が一つもないことが伝わるプレースホルダーを表示する |
