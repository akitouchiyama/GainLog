...
本書は、要件定義書（`docs/Requirements/requirement.md`）7章「データモデル概要」で示されたエンティティ、および認証設計書（`docs/Design/auth.md`）4〜6章・12章で概要のみ定義された `users`・`sessions`・`allowlist` の詳細スキーマを、実装可能な粒度まで具体化するものである。アーキテクチャ設計書（`docs/Design/architecture.md`）7章・9章においても「テーブルスキーマの詳細設計は本書（db.md）で定める」と参照されている。
