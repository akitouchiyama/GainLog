...
- 所有者チェックは D1 に RLS がないためアプリコードの規律に依存する。リポジトリ層への集約・型による `userId` 必須化で機械的強制に寄せる（`auth.md` 13章 → `docs/Design/Detailed/backend.md` 6章で確定済み）。最終防御線となる統合テストの配置は `test.md` で確定する。
