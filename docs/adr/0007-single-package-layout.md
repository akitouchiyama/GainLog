...
単一パッケージ構成を採用する。`src/server`・`src/client`・`src/shared` のようにディレクトリで役割を分ける。
...
- 具体的なディレクトリレイアウト・ビルド構成（Vite + Wrangler）は `docs/Design/Detailed/project-structure.md` で確定する。

## Consequences

- 依存関係管理・ビルド設定・ツールチェーンがシンプルになる。
- パッケージ境界による依存方向の強制（shared が server/client に依存しない等）はできないため、ディレクトリ規約と lint で担保する。
